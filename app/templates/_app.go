...
func init() {
    ...
    router.Handle("/<%= pluralize(entity.name) %>", appstats.NewHandler(handlers.Add<%= _.capitalize(pluralize(entity.name)) %>Handler)).Name("<%= pluralize(entity.name) %>-create").Methods("POST")
